...
def manage():
    ...
    if request.method == 'POST':
        try:
            ...
            if product_no in owner_df['product_no'].values:
                # Update existing product in owner sheet
                owner_df.loc[owner_df['product_no'] == product_no, ['barcode', 'name', 'price', 'cost_price', 'stock', 'description']] = [barcode, name, price, cost_price, stock, description]
                # Update employee sheet (without cost_price)
                employee_df.loc[employee_df['product_no'] == product_no, ['barcode', 'name', 'price', 'stock', 'description']] = [barcode, name, price, stock, description]
            else:
                # Add new product to both sheets
                new_owner_product = pd.DataFrame([{
                ...
                new_employee_product = pd.DataFrame([{
                    'product_no': product_no,
                    'barcode': barcode,
                    'name': name,
                    'price': price,
                ...
                owner_df = pd.concat([owner_df, new_owner_product], ignore_index=True)
                employee_df = pd.concat([employee_df, new_employee_product], ignore_index=True)
            ...
            return render_template('manage.html', 
                                 ...
                                 products=owner_df.to_dict('records'))
        except Exception as e:
            return render_template('manage.html', 
                                 user_type=session['user_type'],
